...
class Client(object):
    ...
    def obtain_certificate(self, domains, old_keypath=None):
        ...
        # Call the proper function for the requested key type
        try:
        ...
        if self.config.dry_run:
            key = key or util.Key(file=None, pem=key_pem)
            csr = util.CSR(file=None, form="pem",
                           data=acme_crypto_util.make_csr(
                               key.pem, domains, self.config.must_staple))
        else:
            key = key or crypto_util.save_key(key_pem, self.config.key_dir)
